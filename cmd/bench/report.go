...
func writeReport(cfg benchmark.Config, results []benchmark.Result) (string, error) {
	...
	name := filepath.Join("reports", fmt.Sprintf("bench_%s_%s.txt", ts, gitRef()))
	...
	renderReport(w, cfg, results)

	return name, nil
}

func renderReport(w io.Writer, cfg benchmark.Config, results []benchmark.Result) {
